 ...
 */
char
ExpandEscapes(esc, endp, unterminated)
	char *esc;
	char **endp;
	int unterminated;
{
	...
	char **endp;
{
	...
	*endp = ptr;
	...
	/*
	 * Move to the start of the tag value
	 */
...
/*
 * Parse an anchor text for the value associated with the
 * passed anchor tag.
 * If the passed tag is not found, return NULL;
 */
char *
ParseAnchor(atext, atag)
	char *atext;
	char *atag;
{
	char *ptr;
	...
	if ((atext == NULL)||(atag == NULL))
	{
		return(NULL);
	}

	ptr = (char *)(atext + strlen(MT_ANCHOR));
 ...
 * (A General version of ParseAnchor)
 */
char *
