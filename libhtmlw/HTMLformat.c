	...
	char *edata;
{
	...
	switch(type)
	{
		case E_TEXT:
			...
			{
				eptr->anchorHRef =
					ParseAnchor(AnchorText, AT_HREF);
				eptr->anchorName =
					ParseAnchor(AnchorText, AT_NAME);
			}
			else
			{
		...
		case E_LINEFEED:
			...
			if (AnchorText != NULL)
			{
				...
				tptr = ParseAnchor(AnchorText, AT_HREF);
				if ((Current != NULL)&&
	...
	unsigned int width;
{
	...
	while (*end != '\0')
	{
		...
		if (start != end)
		{
			...
			tchar = *end;
			*end = '\0';
			ptr = (char *)malloc(strlen(start) + 2);
			if ((NeedSpace == True)&&(stripped_space))
			{
				strcpy(ptr, " ");
				added_space = 1;
			}
		 ...
		 */
		else if ((NeedSpace == True)&&(stripped_space))
		{
			char *tptr;

			XTextExtents(currentFont, " ", strlen(" "), &dir,
				&ascent, &descent, &all);

			tptr = line;
			line = (char *)malloc(strlen(tptr) + strlen(" ") + 1);
			if (line == NULL)
			{
				fprintf(stderr, "Cannot malloc space to compose line\n");
				line = tptr;
 ...
 */
			...
			else
			{
				 ...
				 * not active.
				 */
				tptr = ParseAnchor(mark->start, AT_HREF);
				if (tptr != NULL)
				{
				    /*
	...
	while ((eptr != NULL)&&(eptr->line_number <= (end + 1)))
	{
		...
		else if (eptr->type == E_LINEFEED)
		{
			...
			else if (eptr->next != NULL)
			{
				 ...
				 * Deal with bad Lucidia descents.
				 */
				if (eptr->next->font->descent >
					eptr->next->font->max_bounds.descent)
				{
				    tmpy = eptr->next->y +
					eptr->next->font->max_bounds.ascent +
					eptr->next->font->descent;
				}
				else
				{
				    tmpy = eptr->next->y +
	...
	int lmargin;
{
	...
	while ((eptr != NULL)&&(eptr != end))
	{
		...
		if (eptr->type == E_BULLET)
		{
			...
			char_count = char_count + spaces + 2;

			/*
			 * Double bullet usage for headers to allow
			 * underlining
			 */
			if ((eptr->font == hw->html.header1_font)||
			    (eptr->font == hw->html.header2_font)||
			    (eptr->font == hw->html.header3_font)||
			    (eptr->font == hw->html.header4_font)||
			    (eptr->font == hw->html.header5_font)||
			    (eptr->font == hw->html.header6_font))
			{
				length = length + spaces + 2;
			}
			length = length + spaces + 2;
		}
		else if (eptr->type == E_TEXT)
		{
			...
			char_count = char_count + spaces + elen;

			/*
			 * Double space usage for headers to allow
			 * underlining
			 */
			if ((eptr->font == hw->html.header1_font)||
			    (eptr->font == hw->html.header2_font)||
			    ...
			    (eptr->font == hw->html.header6_font))
			{
				spaces = spaces * 2;
				/*
				 * If a header starts after a list item we
				 * need to force the addition of space for the
				 * bullet here since it wasn't added at the
				 * bullet because we weren't in a header yet.
				 * Hack-o-rama, sigh.
				 *
				 * We know this only happens if we are in a
				 * header and not at a newline.  This means
				 * spaces = 0, and char_count was the length
				 * of the extra space we need before we added
				 * elen to it.
				 */
				if (!newline)
				{
					spaces = char_count - elen;
					if (spaces < 0)
					{
						spaces = 0;
					}
		...
		{
			/*
			 * Double newline usage for headers to allow
			 * underlining
			 */
			if ((eptr->font == hw->html.header1_font)||
			    (eptr->font == hw->html.header2_font)||
	...
	/*
	 * If we end with a fragment of a line that will need to be
	 * underlined, add the space for that underline.
	 */
	if (char_count != 0)
	{
		if ((last->font == hw->html.header1_font)||
		    (last->font == hw->html.header2_font)||
		    (last->font == hw->html.header3_font)||
		    (last->font == hw->html.header4_font)||
		    (last->font == hw->html.header5_font)||
		    (last->font == hw->html.header6_font))
		{
			length = length + 1 + char_count;
		}
	...
	if (text == NULL)
	{
		fprintf(stderr, "ParseTextToPrettyString malloc failed\n");
		return(NULL);
	}
	line_buf = (char *)malloc(max_line + 2);
	if (line_buf == NULL)
	{
		fprintf(stderr, "ParseTextToPrettyString malloc failed\n");
		return(NULL);
