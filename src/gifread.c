...
unsigned char *ReadGifBitmap(fp, datafile, w, h, colrs)
FILE *fp;
char *datafile;
int *w, *h;
XColor *colrs;
...
/* Read in global colormap. */
    ...
    {
	fprintf(stderr, "does not have a colormap\n");
	return(NULL);
    }
...
/*EJB*/
	...
	*h = Height;
	...
	for (i=0; i<ColorMapSize; i++)
	{
		colrs[i].red = Red[i]*65536/scale;
		colrs[i].green = Green[i]*65536/scale;
		colrs[i].blue = Blue[i]*65536/scale;
		colrs[i].pixel = i;
